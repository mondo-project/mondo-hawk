	...
	private boolean transactionalUpdate() throws Exception {
		...
		try (IGraphTransaction t = graph.beginTransaction()) {
			...
			for (final String o : added.keySet()) {
				...
				addEObject(listener, fileNode, addedNodes, addedNodesHash, object);
			}
			...
			for (String s : nodes.keySet()) {
				...
				if (unchanged.containsKey(node.getProperty(IModelIndexer.IDENTIFIER_PROPERTY))) {
					...
					final Iterator<IGraphEdge> itTypeEdges = node.getOutgoingWithType(ModelElementNode.EDGE_LABEL_OFTYPE).iterator();
					final IGraphNode typenode = itTypeEdges.next().getEndNode();
...
//					System.out.println(String.format("type in graph: %s, type in object: %s", nodetype, actualtype));
					node.setProperty(IModelIndexer.SIGNATURE_PROPERTY, o.signature());
					if (!nodetype.equals(actualtype)) {
						// an old ID has been reused with a value of a new type (could be another subclass)
						remove(node, repoURL, fileNode, listener);
						addEObject(listener, fileNode, addedNodes, addedNodesHash, o);
					}
					...
					if (ob.isSet(r)) {
						...
						if (targets instanceof Iterable<?>) {
							...
							if (!((IHawkObject) targets).isProxy())
								...
								addProxyRef(node, (IHawkObject) targets, refname);
							}
						}
	...
	private void addEObject(final IGraphChangeListener listener, final IGraphNode fileNode,
			final Map<IGraphNode, IHawkObject> addedNodes, final Map<String, IGraphNode> addedNodesHash,
			final IHawkObject object) throws Exception {
		final IGraphNode node = inj.addEObject(fileNode, object);
		...
		for (final String transientLabelEdge : ModelElementNode.TRANSIENT_EDGE_LABELS) {
			for (final IGraphEdge e : node.getOutgoingWithType(transientLabelEdge)) {
	...
	private boolean addProxyRef(final IGraphNode node, final IHawkObject destinationObject, final String edgelabel) {
		...
		try {
			...
			if (!destinationObject.URIIsRelative()) {
				...
				destinationObjectRelativePathURI = new DeletionUtils(graph).makeRelative(tempFolderURI,
	...
	private void updateNodeProperties(IGraphNode fileNode, IGraphNode node, IHawkObject eObject) {
		...
		final String nodetype = typenode.getProperty(IModelIndexer.IDENTIFIER_PROPERTY).toString();
		final String actualtype = eObject.getType().getName();
		System.out.println(String.format("type in graph: %s, type in object: %s", nodetype, actualtype));
	...
	private int calculateModelDeltaSize() throws Exception {
		...
		try (IGraphTransaction t = graph.beginTransaction()) {
			...
			boolean modelfilealreadypresent = graph.getFileIndex()
					.get("id", repositoryURL + GraphModelUpdater.FILEINDEX_REPO_SEPARATOR + s.getPath()).iterator()
					.hasNext();
			...
			if (modelfilealreadypresent) {
				...
				t.success();
				System.err.println("updated is of size: " + (delta - newo) + "+" + newo + "+"
	...
	public int resolveProxies(IGraphDatabase graph) throws Exception {
		...
		try (IGraphTransaction tx = graph.beginTransaction()) {
			...
			IGraphIterable<IGraphNode> proxiesToBeResolved = proxyDictionary.query("_proxyRef", "*");
			...
			if (proxiesToBeResolved != null && proxiesToBeResolved.size() > 0) {
				...
				for (IGraphNode n : proxiesToBeResolved) {
					...
					for (String[] proxies : allProxies) {
						String fullPathURI = proxies[0].substring(0, proxies[0].indexOf("#"));
						...
						if (rels != null) {
							...
							if (nodes.size() != 0) {
								...
								for (int i = 0; i < proxies.length; i = i + 2) {
									...
									for (IGraphNode no : nodes) {
										...
										if (nodeURI.equals(proxies[i])) {
											...
											boolean change = new GraphModelBatchInjector(graph, null, listener)
													.resolveProxyRef(n, no, proxies[i + 1]);

											if (!change) {
												System.err.println(
														"resolving proxy ref returned false, edge already existed: "
																+ n.getId() + " - " + proxies[i + 1] + " -> "
																+ no.getId());
											} else {
									...
									if (!found)
										System.err.println(
												"[GraphModelInserter | resolveProxies] Warning: proxy unresolved: "
								...
								System.err.println(
										"[GraphModelInserter | resolveProxies] Warning: no nodes were found for file: "
												+ filePath + " originating in repository " + repoURL + " "
												+ proxies.length / 2 + " proxies cannot be resolved");
						}
					}
				}
	...
	public int resolveDerivedAttributeProxies(IGraphDatabase graph, IModelIndexer m, String type) throws Exception {
		...
		if (size > 0) {
			...
			IAccessListener accessListener = q.calculateDerivedAttributes(graph, allUnresolved);
	...
	public void updateDerivedAttribute(String metamodeluri, String typename, String attributename, String attributetype,
		...
		try (IGraphTransaction tx = graph.beginTransaction()) {
			...
			//
			//
			IAccessListener accessListener = q.calculateDerivedAttributes(graph, derivedPropertyNodes);
